...
```mermaid
flowchart TD
    A[Open Obsidian] --> B[Go to Home Base Note]
    B --> C{What do you need?}
    C -->|Manage Projects| D[Click Efforts → Projects]
    C -->|Capture Ideas| E[Click Plus → Recent Notes]
    C -->|Review Past Work| F[Click Calendar → Daily Notes]
    C -->|Find Knowledge| G[Click Atlas → Reference Areas]
    D --> H[Project Management Dashboard]
    E --> I[Idea Capture Workflow]
    F --> J[Contextual Daily Review]
    G --> K[Knowledge Navigation]
```
